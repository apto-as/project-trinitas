...
---
```
- **Auto-Persona**: Springfield, Krukai, Vector (hierarchical meta-control)
- **MCP Integration**: Sequential (strategic), Context7 (patterns), Magic (UI), Playwright (testing)
- **Tool Orchestration**: [Read, Grep, Glob, Bash, TodoWrite, Edit, MultiEdit, Task]
- **Arguments**: `[operation]`, `[target]`, `@<path>`, `--trinitas-brief`, `--trinitas-focus [strategy|technical|risk]`
- **Operations**: analyze, implement, design, improve, plan, assess, audit, troubleshoot
...
- **Version-Control**: git
- **Meta**: index, load, spawn, sc:trinitas

### Wave-Enabled Commands
8 commands: `/analyze`, `/build`, `/design`, `/implement`, `/improve`, `/task`, `/workflow`, `/sc:trinitas`
